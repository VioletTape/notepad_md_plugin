...
// Base HTML embedded as a wide raw string literal.
// The MARKED_JS_PLACEHOLDER comment marks where marked.js is injected at build time.
static const wchar_t* BASE_HTML = LR"html(<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<style>
...
h1,h2,h3,h4,h5,h6 { font-weight: 600; margin: 24px 0 8px; line-height: 1.25; color: #FF8040; }
...
p  { margin: 0 0 16px; }
code { font-family: 'Cascadia Code', 'Fira Code', Consolas, monospace; font-size: 93%;
       background: #161b22; padding: 0.2em 0.4em; border-radius: 3px; color: #c9cbff; }
pre  { background: #29323F; padding: 16px; border-radius: 6px; overflow: auto; margin-bottom: 16px; }
pre code { background: none; padding: 0; color: inherit; }
blockquote { border-left: 3px solid #4d9be6; background: #1e2530; padding: 12px 16px; color: #e6edf3; margin-bottom: 16px; border-radius: 0 4px 4px 0; }
...
abbr[title] { text-decoration: underline dotted #8b949e; cursor: help; color: #79c0ff; }
img { max-width: 100%; }
...
function parseFrontMatterAcronyms(md) {
  var fm = md.match(/^\s*---\n([\s\S]*?)\n---/);
  if (!fm) return {};
  var section = fm[1].match(/^acronyms:\s*\n((?:[ \t]+[^\n]+\n?)*)/m);
  if (!section) return {};
  var result = {};
  section[1].replace(/[ \t]+([^:\n]+?)\s*:\s*([^\n]+)/g, function(_, k, v) {
...
function applyAcronyms(el, acronyms) {
  ...
  var re = new RegExp('(?<![A-Za-z0-9-])(' + escaped.join('|') + ')(?![A-Za-z0-9-])', 'g');
  var walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
    acceptNode: function(n) {
      var p = n.parentNode;
      while (p && p !== el) {
        var t = p.tagName;
        if (t === 'CODE' || t === 'PRE' || t === 'ABBR' || t === 'A') return NodeFilter.FILTER_REJECT;
        p = p.parentNode;
      }
  ...
  if (typeof marked !== 'undefined') {
    ...
    el.querySelectorAll('blockquote').forEach(function(bq) {
      ...
      if (first && first.textContent.startsWith('!')) {
        first.innerHTML = first.innerHTML.replace(/^!(\s?)/, '');
        bq.style.borderLeftColor = '#c94f4f';
      }
    });
...
void PreviewWindow::OnBufferActivated() {
    if (!IsVisible()) return;
